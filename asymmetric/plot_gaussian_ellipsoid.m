function h = plot_gaussian_ellipsoid(m, C, sdwidth, npts, axh, positionVector)
...
switch numel(m)
   ...
   case 3, h=show3dAsym(m(:),C,sdwidth,npts,axh,positionVector);
   otherwise
...
%-----------------------------
function h = show3dAsym(means, C, sdwidth, npts, axh, positionVector)
...
yp = reshape(bp(2,:), size(y));
if positionVector(2) == 0
    yp(yp>means(2)) = NaN;
else
    yp(yp<means(2)) = NaN;
end
zp = reshape(bp(3,:), size(z));
if positionVector(3) == 0
    zp(zp>means(3)) = NaN;
else
    zp(zp<means(3)) = NaN;
end
